...
void app_main(void)
{
    int a;
    void * ctx = &a;
    wifi_csi_config_t csiConfig = {.lltf_en=true, .htltf_en=true, .stbc_htltf2_en=true, .ltf_merge_en=true, .channel_filter_en=true, .manu_scale=false, .shift=0};
    ...
    /**
     * @brief Initialize Wi-Fi csi
     */
    esp_wifi_set_csi_rx_cb(wifi_csi_raw_cb, ctx);
    esp_wifi_set_csi_config(&csiConfig);
    esp_wifi_set_csi(true);

    // ping AP from the station
    gw.addr = "192.168.4.1";
